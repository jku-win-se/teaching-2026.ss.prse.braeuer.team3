...
### Java Code Quality Rules (Project-Specific)

**PMD Compliance (NFR-04):**
- Review generated Java code against common PMD violations:
  - No empty catch blocks (`EmptyCatchBlock`)
  - No unused imports or variables (`UnusedImports`, `UnusedLocalVariable`)
  - No god classes or excessively long methods (`GodClass`, `ExcessiveMethodLength`)
  - No `System.out.println` in production code (`SystemPrintln`)
  - Exception handling must use specific exception types, not just `Exception`
- After each code generation step: perform a mental PMD review of the generated code

**Javadoc (NFR-06):**
- Every generated `public` class must have a Javadoc block directly above the class declaration
- Every generated `public` method must have Javadoc including:
  - A descriptive first sentence (purpose)
...
- Every generated `public` interface must have Javadoc at both the interface and method level
- Applies explicitly to all classes in `domain/`, `service/`, `repository/` (interfaces), `controller/`, `api/`
