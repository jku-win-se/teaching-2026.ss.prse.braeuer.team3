...
> **Project-specific (SmartHome Orchestrator):** Build instructions must include a PMD check and Javadoc validation step (NFR-04, NFR-06). See section below.
...
### PMD Check (NFR-04)
```bash
# Run PMD via Maven (build fails on critical/high violations)
mvn pmd:check

# Generate PMD report only, without failing the build
mvn pmd:pmd
```
- Report path: `target/pmd.xml` / `target/site/pmd.html`
- Configuration: `ruleset.xml` in the project root
- **Build fails** on critical or high violations

### Javadoc Validation (NFR-06)
```bash
# Generate Javadoc and check for warnings/errors
mvn javadoc:javadoc

# Javadoc check (fails on missing docs if configured accordingly)
mvn javadoc:check
```
- Report path: `target/site/apidocs/`
- Missing Javadoc on public classes/methods in the Domain/API layer must be treated as a blocker
...
## Code Quality Checks (Project-Specific)
- [ ] PMD: 0 critical violations
- [ ] PMD: 0 high violations
- [ ] Javadoc: All public classes in the Domain/API layer are documented
- [ ] Javadoc build: No Javadoc errors
