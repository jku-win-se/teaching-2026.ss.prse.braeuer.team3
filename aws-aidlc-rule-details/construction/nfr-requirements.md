...
### Project-Specific NFR Constraints (derived from Requirements — no questions needed)

**PMD (NFR-04):**
- No critical or high PMD violations allowed in the codebase
- PMD check runs automatically on every CI build (configured via `ruleset.xml` in the project root)
- Build fails on critical/high findings
- Relevant PMD rulesets: `category/java/bestpractices.xml`, `category/java/errorprone.xml`, `category/java/design.xml`

**Javadoc (NFR-06):**
- All `public` classes, interfaces, and methods in the Core Domain Layer (e.g. `domain/`, `service/`) must have Javadoc
- All `public` classes, interfaces, and methods in the API Layer (e.g. `controller/`, `api/`) must have Javadoc
- Minimum Javadoc content: `@param` for all parameters, `@return` for non-void methods, one descriptive sentence stating the purpose
