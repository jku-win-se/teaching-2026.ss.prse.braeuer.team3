  ...
  template: `
    <div class="auth-page">
      <div class="auth-brand">
        ...
        <h1>SmartHome</h1>
        <p>Set up your smart home account.</p>
      </div>
        ...
        <mat-card-content>
          ...
          <form [formGroup]="form" (ngSubmit)="submit()" style="display:flex;flex-direction:column;gap:16px;margin-top:24px;">
            <mat-form-field appearance="outline">
              ...
              <mat-icon matPrefix style="margin-right:8px;color:var(--text-muted);">person_outline</mat-icon>
              <input matInput formControlName="name" placeholder="Alex Miller" autocomplete="name">
            ...
            <div *ngIf="form.get('password')?.value" style="margin-top:-10px;">
              <div style="display:flex;gap:4px;margin-bottom:4px;">
                <div *ngFor="let s of [1,2,3,4]"
                  style="height:4px;flex:1;border-radius:4px;transition:background 0.3s;"
                  [style.background]="strengthColor(s)">
                </div>
              </div>
...
<div *ngIf="errorMsg" style="background:#FEF2F2;border:1px solid #FECACA;border-radius:8px;padding:10px 14px;display:flex;align-items:center;gap:8px;">
          ...
          <p style="text-align:center;margin-top:24px;font-size:14px;color:var(--text-muted);">
            ...
            <a routerLink="/login" style="color:var(--primary);font-weight:600;text-decoration:none;"> Sign in</a>
          </p>
        </mat-card-content>
      </mat-card>
    </div>
...
export class RegisterComponent {
  ...
  strengthColor(step: number): string {
    const score = this.strengthScore;
    if (step > score) return '#E2E8F0';
